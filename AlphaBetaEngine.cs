...
namespace FourInARow
{
    class AlphaBetaEngine : Player
    {
        ...
        private int abSearchMax(int depth, int alpha, int beta)
        {
            ...
            if (depth <= 0) {
                Console.WriteLine("max depth " + depth.ToString() + " " + eval(g).ToString());
                return eval(g);
            }
            List<int> moves = new List<int>(g.possibleMoves);
            ...
            foreach (var item in moves)
            {
                ...
                g.undoMove(item);
                if (bestValue >= beta) {
                    Console.WriteLine("max depth " + depth.ToString() + " " + bestValue.ToString());
                    return bestValue;
                }
                a = Math.Max(a, value);
            }
        ...
        private int abSearchMin(int depth, int alpha, int beta)
        {
            ...
            if (depth <= 0) {
                Console.WriteLine("min depth 0 " + eval(g));
                return eval(g);
            }
            List<int> moves = new List<int>(g.possibleMoves);
            int value, bestMove = -1, bestValue = 2000;
            foreach (var item in moves)
            {
                ...
                g.undoMove(item);
                if (bestValue <= alpha) {
                    Console.WriteLine("min depth " + depth.ToString() + " " + bestValue.ToString());
                    return bestValue;
                }
                b = Math.Min(b, value);
            }
